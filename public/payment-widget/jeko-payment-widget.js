...
//             if (!info) return "";
//             return `Contrat n° ${info.idProposition || info.contratIdWeb} — ${info.produit || ""}  — ${info.souscripteur || this._paymentData.customerName || ""}`;
//         }
...
//         animation: jeko-fade-in .18s ease;
//       }
//       @keyframes jeko-fade-in { from { opacity:0 } to { opacity:1 } }
//       @keyframes jeko-slide-up { from { opacity:0; transform:translateY(10px) } to { opacity:1; transform:translateY(0) } }
...
//         animation: jeko-slide-up .22s cubic-bezier(.16,1,.3,1);
//       }
...
//         position:relative;
//       }
//       .jeko-header-top { display:flex; align-items:center; justify-content:space-between; margin-bottom:16px; }
//       .jeko-header h3 { margin:0; font-size:17px; font-weight:700; letter-spacing:.2px; }
//       .jeko-close {
...
//       }
//       .jeko-close:hover { background:rgba(255,255,255,.28); }

//       /* Stepper */
//       .jeko-steps { display:flex; align-items:center; gap:0; }
//       .jeko-step { display:flex; align-items:center; flex:1; }
//       .jeko-step:last-child { flex:0; }
//       .jeko-step-circle {
...
//       }
//       .jeko-step-label {
//         font-size:11px; margin-left:8px; color:rgba(255,255,255,.7);
//         white-space:nowrap; font-weight:600;
//       }
//       .jeko-step-line {
//         flex:1; height:2px; background:rgba(255,255,255,.25);
//         margin:0 10px; border-radius:2px; transition:background .2s ease;
//       }
//       .jeko-step.active .jeko-step-circle {
//         background:#fff; color:var(--jeko-primary-dark); border-color:#fff;
//       }
//       .jeko-step.active .jeko-step-label { color:#fff; }
//       .jeko-step.done .jeko-step-circle {
//         background:var(--jeko-accent); color:#1f2937; border-color:var(--jeko-accent);
//       }
//       .jeko-step.done .jeko-step-line { background:var(--jeko-accent); }
//       .jeko-step.done .jeko-step-label { color:rgba(255,255,255,.9); }

//       /* Body */
//       .jeko-body { padding:24px; overflow-y:auto; flex:1; animation: jeko-slide-up .2s ease; }
//       .jeko-body::-webkit-scrollbar { width:6px; }
//       .jeko-body::-webkit-scrollbar-thumb { background:#d1d5db; border-radius:6px; }

//       .jeko-amount { text-align:center; padding:4px 0 20px; }
//       .jeko-amount .amount { font-size:30px; font-weight:800; color:var(--jeko-primary-dark); display:block; letter-spacing:-.5px; }
//       .jeko-amount .description { font-size:13px; color:#6b7280; margin-top:4px; display:block; }

//       .jeko-section-label { font-size:13px; font-weight:700; color:#374151; margin:0 0 10px; display:block; text-align:left; }

//       /* Payment methods grid */
//       .jeko-methods { display:grid; grid-template-columns:repeat(auto-fill,minmax(130px,1fr)); gap:10px; }
//       .jeko-method-btn {
//         border:2px solid #e5e7eb; background:#fff; border-radius:12px; padding:14px 10px;
//         text-align:center; cursor:pointer; width:100%;
//         display:flex; flex-direction:column; align-items:center; gap:6px;
//         transition:all .15s ease;
//       }
//       .jeko-method-btn:hover:not(.cards-disabled) { border-color:#c9d8ce; background:#fafdfb; transform:translateY(-1px); }
//       .jeko-method-btn[aria-pressed="true"] { border-color:var(--jeko-primary); background:#eef7f1; box-shadow:0 0 0 3px rgba(29,96,61,.1); }
//       .jeko-method-btn.cards-disabled { background:#f5f5f5; cursor:not-allowed; opacity:.55; }
...
//       /* Contract lookup */
//       .jeko-lookup-icon {
//         width:56px; height:56px; border-radius:50%; background:#eef7f1;
//         display:flex; align-items:center; justify-content:center;
...
//         border:2px solid #e5e7eb; font-size:15px; font-family:inherit; transition:border-color .15s;
//       }
//       .jeko-field input:focus { outline:none; border-color:var(--jeko-primary); }
//       .jeko-field input.error { border-color:#dc2626; }
//       .jeko-field .error-message { color:#dc2626; font-size:12px; margin-top:5px; display:none; }
//       .jeko-field .error-message.visible { display:block; }
//       .disabled-input { background-color:#f3f4f6 !important; color:#9ca3af !important; cursor:not-allowed; pointer-events:none; }

//       /* Stepper control */
//       .jeko-stepper { display:flex; align-items:center; gap:14px; }
//       .jeko-stepper button {
//         width:38px; height:38px; border-radius:10px; border:2px solid #e5e7eb; background:#fff;
...
//       .jeko-stepper span { font-size:17px; font-weight:800; min-width:26px; text-align:center; color:#111827; }

//       /* Invoice list */
//       .jeko-invoice-list { display:flex; flex-direction:column; gap:8px; max-height:220px; overflow-y:auto; }
//       .jeko-invoice-item {
//         display:flex; align-items:center; gap:10px; border:2px solid #e5e7eb; border-radius:10px;
//         padding:11px 12px; cursor:pointer; transition:border-color .15s;
//       }
//       .jeko-invoice-item:hover { border-color:#c9d8ce; }
//       .jeko-invoice-item .meta { flex:1; font-size:12.5px; color:#374151; }
//       .jeko-invoice-item .amount { font-weight:700; font-size:13px; color:#111827; }
//       .jeko-invoice-item input[type="checkbox"] { width:17px; height:17px; accent-color:var(--jeko-primary); cursor:pointer; }

//       .jeko-total-bar {
//         display:flex; justify-content:space-between; align-items:center; margin-top:14px;
//         padding:12px 14px; background:#f0f7f3; border-radius:10px; font-weight:700; font-size:14px;
//       }

//       /* Notices */
//       .jeko-notice { font-size:12.5px; padding:11px 14px; border-radius:10px; margin-bottom:14px; line-height:1.4; }
//       .jeko-notice.warn { background:#fef2f2; color:#991b1b; border:1px solid #fecaca; }
...
//       /* Toggle */
//       .jeko-toggle-group { display:flex; gap:10px; margin-top:6px; }
//       .jeko-toggle-btn {
//         flex:1; border:2px solid #e5e7eb; background:#fff; border-radius:10px; padding:11px 14px;
//         text-align:center; cursor:pointer; font-family:inherit; font-weight:700; font-size:13.5px;
//         transition:all .15s;
//       }
//       .jeko-toggle-btn:hover { border-color:#c9d8ce; }
//       .jeko-toggle-btn[aria-pressed="true"] { border-color:var(--jeko-primary); background:#eef7f1; color:var(--jeko-primary-dark); }

//       /* Breakdown */
//       .jeko-breakdown { background:#f9fafb; border-radius:12px; padding:14px 16px; margin-top:14px; border:1px solid #f0f0f0; }
//       .jeko-breakdown-row { display:flex; justify-content:space-between; padding:5px 0; font-size:13px; color:#4b5563; }
//       .jeko-breakdown-row.total { font-weight:800; color:#111827; border-top:2px solid #e5e7eb; padding-top:9px; margin-top:5px; font-size:14px; }

//       /* Summary step (step 3) */
//       .jeko-summary-card { border:2px solid #eef1ef; border-radius:14px; overflow:hidden; }
//       .jeko-summary-row {
...
//       }
//       .jeko-summary-total .label { font-size:12px; opacity:.85; font-weight:600; letter-spacing:.3px; text-transform:uppercase; }
//       .jeko-summary-total .amount { font-size:28px; font-weight:800; margin-top:4px; letter-spacing:-.5px; }

//       /* Footer / actions */
//       .jeko-footer { padding:16px 24px 22px; border-top:1px solid #f0f0f0; display:flex; gap:10px; }
//       .jeko-btn-back {
...
//         padding:14px; border-radius:12px; cursor:pointer; font-family:inherit; transition:filter .15s, transform .1s;
//       }
//       .jeko-submit:hover:not(:disabled) { filter:brightness(1.05); }
//       .jeko-submit:active:not(:disabled) { transform:scale(.99); }
//       .jeko-submit:disabled { opacity:.5; cursor:not-allowed; }
//       .jeko-submit.outline {
//         background:#fff; color:var(--jeko-primary-dark); border:2px solid var(--jeko-primary);
//       }

//       /* State screens */
//       .jeko-state { padding:44px 24px; text-align:center; min-height:220px; display:flex; flex-direction:column; align-items:center; justify-content:center; }
//       .jeko-state h4 { margin:4px 0 6px; font-size:17px; color:#111827; }
//       .jeko-state p { margin:0; font-size:13.5px; color:#6b7280; max-width:280px; }
//       .jeko-state .icon { font-size:52px; margin-bottom:10px; }
//       .jeko-spinner {
//         width:42px; height:42px; border:3px solid #e5e7eb; border-top-color:var(--jeko-primary);
...
//         font-weight:700; padding:11px 26px; border-radius:10px; cursor:pointer; font-family:inherit; transition:background .15s;
//       }
...
//           <div class="error-message">${t.requiredField}</div>
//         </div>
//         <button type="button" class="jeko-submit" data-action="verify-contract" style="margin-top:16px;width:100%;background:#fff;color:var(--jeko-primary-dark);border:2px solid var(--jeko-primary);">${t.verify}</button>
//       `;
//         }
